...
struct ProgramState {
    ...
    ProgramState()
            : camera(glm::vec3(1.0f, -1.0f, -4.0f)) {}
...
int main() {
    ...
    dirLight.direction = glm::vec3(-0.965f, -1.0f, -0.7f);
    dirLight.ambient =   glm::vec3(-0.3f, 0.2f, 0.3f);
    dirLight.diffuse =   glm::vec3(0.8f, 0.5f, 0.9f);
    dirLight.specular =  glm::vec3(0.2f, 7.1f, 1.3f);
   ...
   //plane
    ...
    // load textures
    // -------------
    unsigned int terrainTexture = loadTexture(FileSystem::getPath("resources/textures/mapa.jpg").c_str());
    ...
    vector<std::string> faces
            {
                    FileSystem::getPath("resources/textures/novi1/right.jpg"),
                    FileSystem::getPath("resources/textures/novi1/left.jpg"),
                    FileSystem::getPath("resources/textures/novi1/top.jpg"),
                    FileSystem::getPath("resources/textures/novi1/bottom.jpg"),
                    FileSystem::getPath("resources/textures/novi1/front.jpg"),
                    FileSystem::getPath("resources/textures/novi1/back.jpg")
            };
    unsigned int cubemapTexture = loadCubemap(faces);



    //Framebuffers setup(Bloom)
    unsigned int hdrFBO;
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        ourShader.setFloat("material.shininess", 19.0f);
      ...
      // glCullFace(GL_BACK);
       ...
       model = glm::translate(model,programState->modelPosition);
        ...
        terrainShader.setVec3("viewPosition", programState->camera.Position);
        ...
        {
            model = glm::mat4(2.0f);
            model = glm::translate(model, flames[i]);
        ...
        // We blur bright fragments with two-pass Gaussian Blur
        // --------------------------------------------------
        glActiveTexture(GL_TEXTURE0);
        bool horizontal = true, first_iteration = true;
        unsigned int amountBlur = 5;
        blurShader.use();
